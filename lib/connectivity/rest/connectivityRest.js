...
var rest = module.exports = {
    ...
    checkUserStatus: function(accessToken, connectivityInstanceId, userId, options) {
        ...
        var url = config.backtory.newConnectivityApi + "check-user-status/";
        unirest.get(url)
            .headers({
            ...
            .end(function (response){
                ...
                if (response.status == 200) {
                    options.success(response.body, config.backtory.newConnectivityApi);
                }
                else {
                    options.error({
    ...
    deleteChatGroup: function(accessToken, connectivityInstanceId, groupId, options) {
        var old_time = new Date();
        var url = config.backtory.connectivityApi + "chat/group/";
        unirest.delete(url)
            .headers({
                'Accept': 'application/json',
                'Content-Type': 'application/json',
    ...
    sendMatchResult: function(accessToken, connectivityInstanceId, matchId, winners, extraData, options) {
        var old_time = new Date();
    ...
    startMatch: function(accessToken, connectivityInstanceId, matchId, options) {
        var old_time = new Date();
        var url = config.backtory.connectivityApi + "challenge/" + matchId + "/start-match/";
        unirest.post(url)
            .headers({
                'Accept': 'application/json',
                'Content-Type': 'application/json',
