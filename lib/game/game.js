...
game.EventBulk.prototype.add = function (event, userId) {
    if ((typeof userId != "string") || !(userId)) {
        throw new Error("Second argument (userId) must be a valid string.")
    }
    if (!(event instanceof game.Event)) {
        throw new Error("First argument (event) must be a valid event object.")
    }
    this.eventUserData.push({"userId": userId, "event": event});
};


game.EventBulk.prototype.send = function (options) {
    var eventData = this.eventUserData;
    var jsonEvent = {'eventBunchDataList': []};
    for (var i = 0; i < eventData.length; i++) {
        jsonEvent['eventBunchDataList'].push({'eventName': eventData[i].event.eventName, 'fieldsAndValues': eventData[i].event.fieldsAndValues,
    ...
    game.common.getInstance("game", function (gameInstanceId) {
        game.common.getAccessToken({
            ...
            success: function (accessToken) {
                gameRest.sendEventBulk(accessToken, gameInstanceId, jsonEvent, {
                    success: function() {
...
game.LeaderBoard.prototype.remove = function(options) {
    ...
    game.common.getInstance("game", function (gameInstanceId) {
        game.common.getAccessToken({
            ...
            success: function (accessToken) {
                gameRest.removeLeaderBoard(accessToken, gameInstanceId, leaderBoardId, {
                    success: function() {
                        if (options && options.success) {
                            options.success();
                        }
...
game.LeaderBoard.prototype.reset = function(options) {
    var leaderBoardId = this.leaderBoardId;
    game.common.getInstance("game", function (gameInstanceId) {
        game.common.getAccessToken({
            error: function (error1) {
            ...
            success: function (accessToken) {
                gameRest.resetLeaderBoard(accessToken, gameInstanceId, leaderBoardId, {
                    success: function() {
                        if (options && options.success) {
                            options.success();
                        }
...
game.Game.resetAllLeaderBoards = function (options) {
    game.common.getInstance("game", function (gameInstanceId) {
        game.common.getAccessToken({
            error: function (error1) {
                if (options && options.error) options.error(error1);
            ...
            success: function (accessToken) {
                gameRest.resetAllLeaderBoards(accessToken, gameInstanceId, {
                    success: function() {
                        if (options && options.success) {
                            options.success();
                        }
