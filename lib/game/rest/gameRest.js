...
var rest = module.exports = {
    ...
    sendEventBulk: function(accessToken, gameInstanceId, jsonEvent, options) {
        var old_time = new Date();
        var url = config.backtory.gameApi + "events/bulk";
        unirest.post(url)
            .headers({
                'Accept': 'application/json',
                'Content-Type': 'application/json',
            ...
            .end(function (response){
                ...
                if (config.debug) console.log("time spent: " + seconds_passed);
                if (response.status == 200) {
                    if (config.debug) console.log("sendEventBulk response is " + JSON.stringify(response.body) + "\n");
                    options.success();
                }
                else {
                    options.error({
                ...
                sentry.checkAPICall("game", gameInstanceId, "SEND_EVENT_BULK",
                    response.status, response.body, seconds_passed, url, "POST");
            });
    },
