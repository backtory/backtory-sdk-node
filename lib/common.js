...
var readIntegrationInfo = function(fileName, callback) {
    fs.readFile(fileName, 'utf8', function (err,data) {
        ...
        if (process.env.BacktoryDataBaseId) common.instances["database"] = process.env.BacktoryDataBaseId;
        if (process.env.BacktoryAuthenticationId) common.instances["auth"] = process.env.BacktoryAuthenticationId;
        if (process.env.BacktoryGameId) common.instances["game"] = process.env.BacktoryGameId;
        if (process.env.BacktoryCloudCodeId) common.instances["cloud-code"] = process.env.BacktoryCloudCodeId;
        if (process.env.BacktoryMasterKey) common.masterKey = process.env.BacktoryMasterKey;
