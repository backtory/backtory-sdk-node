...
var queryMethods = module.exports = function (objectStorage, objectStorageRest) {
    ...
    // Added By Mohammad Mesbah.
    objectStorage.Query.prototype.sample = function(num) {
        if (typeof num != "number")
            throw new Error("First argument of 'sample' method must be a number.");
        if (num<=0)
            throw new Error("First argument of 'sample' method must be greater than 0.");
        if (num>1000)
            throw new Error("First argument of 'sample' method must be less than or equal 1000.");
        this.params["sample"] = num;
    };
    // End edition by Mesbah.
        ...
        objectStorage.common.getInstance("database", function (objectStorageInstanceId) {
            objectStorage.common.getAccessToken({
                ...
                success: function (accessToken) {
                    // Added by Mohammad Mesbah.
                    if (query.params["sample"] != undefined) {
                        delete query.params["count"];
                        delete query.params["skip"];
                        delete query.params["limit"];
                    }
                    // End edition by Mesbah.
